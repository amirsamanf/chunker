...
def prepare_semiChar(seq):

    semiChar = []
    for word in seq:
        emptyVect = [0 for i in range(100)]
        allChars = string.printable
        ind1 = allChars.find(word[0])
        v1 = emptyVect
        v1[ind1] = 1

        v3 = emptyVect
        ind2 = allChars.find(word[-1])
        v3[ind2] = 1

        v2 = emptyVect
        for i in range(1, len(word)-1):
            charInd = allChars.find(word[i])
            v2[charInd] += 1

        v1T = torch.tensor(v1, dtype=torch.long)
        v2T = torch.tensor(v2, dtype=torch.long)
        v3T = torch.tensor(v3, dtype=torch.long)

        semiChar.append(torch.cat([v1T, v2T, v3T], dim=0))

    stacked = torch.stack(semiChar, dim=1)
    return stacked
...
class LSTMTaggerModel(nn.Module):
    ...
    def forward(self, sentence, semi):
        embeds = self.word_embeddings(sentence)

        completeEmbed = torch.cat([embeds, semi.transpose_(0, 1)], dim=1)

        lstm_out, _ = self.lstm(completeEmbed.view(len(sentence), 1, -1))
...
class LSTMTagger:
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            ...
            semi = prepare_semiChar(seq).float()

            tag_scores = self.model(inputs, semi)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                semi_in = prepare_semiChar(sentence).float()

                targets = prepare_sequence(tags, self.tag_to_ix, self.unk)

                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in, semi_in)
